...
double compute_distance(const geometry_msgs::msg::Point & start, const geometry_msgs::msg::Point & goal)
{
  return std::hypot(goal.x - start.x, 
                    goal.y - start.y);
}

double compute_angle_distance(const geometry_msgs::msg::Quaternion & start, const geometry_msgs::msg::Quaternion & goal)
{
